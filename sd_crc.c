...
uint16_t sd_crc16_byte(uint16_t crcval, uint8_t byte)
{
...
uint8_t sd_compute_crc7(uint8_t *data, uint16_t size)
{
    ...
    {
        //do a single byte
        register uint8_t c = *(data++);
        register uint16_t i = 8;
        do
        {
            crc_7 <<= 1;
            if(c & 0x80)
...
uint16_t sd_compute_crc16(uint8_t *data, uint16_t size)
{
    ...
    do
    {
        crc_16 = sd_crc16_byte(crc_16, *(data++));
    }
